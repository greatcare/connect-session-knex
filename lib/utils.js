...
async function isDbSupportJSON(knex) {
  ...
  if (/MariaDB/.test(version)) {
    return isMariaDBSupportJSON(version);
  }
...
function isMariaDBSupportJSON(_version) {
  // According to MariaDB docs, the JSON type is just an alias that's equivalent
  //  to LONGTEXT, so we may just as well use text-based fields instead.
  return false;
}
