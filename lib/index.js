...
// We need to cap `sid` column size because otherwise, index creation can fail on MariaDB 10.1 with:
//  > ER_INDEX_COLUMN_TOO_LONG: Index column size too large. The maximum column size is 767 bytes.
          ...
          return new Promise((res) => {
            isDbSupportJSON(self.knex).then((isSupport) => {
              return self.knex.schema.createTable(self.tablename, (table) => {
                table.string(self.sidfieldname, sessionIDLength).primary();
                if (isSupport) {
